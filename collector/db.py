...
async def insert_sensor(ble_id: str) -> int:
    ...
    if EXECUTION_MODE in ("OFFLINE_SIMULATION", "MOCK_INSERT"):
        ...
        # Format all records into a single, compact line
        records_summary = ", ".join([f"[{m['ble_id']} (ID:{m['sensor_id']}) -> {m['temperature']}°C/{m['humidity_raw']}%]" for m in buffer])
...
async def db_worker() -> None:
    ...
    try:
        while True:
            ...
            if now_dt.second == 0 and minute_accumulator:
                # Force-truncate metrics timestamp to minute-precision for optimal hypertable bucket alignments
                timestamp_bucket = now_dt.replace(second=0, microsecond=0)
                ...
                for ble_id, metrics in minute_accumulator.items():
                    ...
                    aggregated_data = {
                        "ble_id": ble_id,
                        "sensor_id": _mapping_cache[ble_id]["sensor_db_id"],
                        "temperature": avg_temp,
                        "humidity_raw": avg_hum,
                        "battery_raw": avg_bat,
                        "time": timestamp_bucket
                    }

                    buffer.append(aggregated_data)
                    last_state[ble_id] = aggregated_data.copy()
                ...
                # Anti-bounce safety trigger: yield loop execution context briefly to step cleanly past the 00-second window
